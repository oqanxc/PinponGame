...
function impact(a,b){//topun dikdörtgenlere çarpma durumuna göre true/false değeri döndürüldü.topun ve çubuğun  en sağ-sol, yukarı-aşağı değerleri bulunarak işlem taamlandı.
    a.top=a.y-a.r;
    a.bottom=a.y+a.r;
    a.left=a.x-a.r;
    a.right=a.r+a.x;
...
function movement(e) {//fare kontrolü için event listener oluşturuldu
...
function update(){//ekran yenilemesi için update fonsiyonu tanımlandı daha sonra ne kadar sürede yenileceğine setInterval fonksiyonu ile karar verildi
    ...
    if(impact(ball,player)==true){
    ball.velocitX=-ball.velocitX;
        ball.speed+=5;
    }
    if(ball.x-ball.r>cvs.width){
    user.score++;
    RestartGame();    
}
